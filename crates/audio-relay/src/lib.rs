...
mod tests {
    ...
    fn pcm_written_to_the_fifo_arrives_on_the_socket_and_the_fifo_is_removed_after() {
        ...
        let took = t0.elapsed();
        let real_time = Duration::from_secs_f64(payload.len() as f64 / BYTES_PER_SECOND as f64);
        assert!(took >= real_time.mul_f64(0.8), "paced at the format's rate: {took:?} for {real_time:?} of audio");
